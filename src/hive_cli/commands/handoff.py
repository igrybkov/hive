...
def show_all_handoffs() -> None:
    ...
    all_handoffs = handoffs.list_handoffs(main_repo)
    ...
    active_handoffs = [
        (b, f) for b, f in all_handoffs if handoffs.has_handoff_content(f)
    ]
...
def show_handoff(branch: str) -> None:
    ...
    """
    main_repo = get_main_repo()
    handoff_file = handoffs.get_handoff_file(branch, main_repo)
    _format_handoff_preview(branch, handoff_file)
...
def create_handoff(
    ...
    if commit_changes and is_worktree_dirty(worktree_path):
        ...
        if confirm("Create WIP commit?"):
            if handoffs.create_wip_commit(worktree_path):
    ...
    # Get current state
    last_commit = handoffs.get_last_commit(worktree_path)
...
def edit_handoff(branch: str) -> None:
    ...
    if not handoff_file.exists() or not handoffs.has_handoff_content(handoff_file):
...
def clear_handoff(branch: str) -> None:
    ...
    if handoff_file.exists() and handoffs.has_handoff_content(handoff_file):
...
def _complete_branch(ctx, param, incomplete):
    ...
    try:
        ...
        handoff_entries = handoffs.list_handoffs(main_repo)
        branches = [b for b, _ in handoff_entries]
...
def list_cmd(
    ...
    for branch, handoff_file in all_handoffs:
        has_content = handoffs.has_handoff_content(handoff_file)
...
def show(
    ...
    """
    if not branch:
        ctx = handoffs.get_current_branch_context()
        if not ctx:
...
def _resolve_create_target(branch: str | None) -> tuple[str, Path] | None:
    """Resolve (branch, worktree_path) for `create`, given an optional override.
...
def _prompt_multiline_message() -> str | None:
    """Prompt for a multi-line handoff summary on an interactive terminal."""
...
def create(
    ...
    """
    target = _resolve_create_target(branch)
    if not target:
        error("Not in a git repository")
        sys.exit(1)
    branch, worktree_path = target
    ...
    if not message_str and sys.stdin.isatty():
        message_str = _prompt_multiline_message()
...
def clean(
    ...
    if dry_run:
        orphaned = handoffs.preview_orphaned_handoffs(main_repo)
        if orphaned:
            console.print("[yellow]Would remove:[/]")
            for branch, orphan_path in orphaned:
                console.print(f"  {branch}: {orphan_path}")
        else:
            info("No orphaned handoffs to clean")
        return

    removed = handoffs.clean_orphaned_handoffs(main_repo)
