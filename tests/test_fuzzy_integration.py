...
Unlike ``tests/test_fuzzy.py`` (which mocks ``Application`` entirely), these
...
from hive_cli.utils.fuzzy import FuzzyItem, _fuzzy_match, fuzzy_select
...
def _run_with_input(inp: PipeInput, **kwargs: Any) -> str | None:
    ...
    with (
        ...
        patch("hive_cli.utils.fuzzy.create_output", return_value=DummyOutput()),
    ):
